...
void Game::RenderSnake(Snake *snake_ptr) {
    ...
    for (int i = 0; i < snake_ptr->field_height * snake_ptr->field_width; i++) {
        if (snake_ptr->body.head[X_COORDINATE] * tile_size_.x == tile_map_[i].getPosition().x && ///head
            ...
            int x;
            int y;
            if (snake_ptr->body.current_direction == Direction::DIRECTION_UP) {
                x = 3;
                y = 0;
            } else if (snake_ptr->body.current_direction == Direction::DIRECTION_DOWN) {
                x = 4;
                y = 1;
            } else if (snake_ptr->body.current_direction == Direction::DIRECTION_LEFT) {
                x = 3;
                y = 1;
            } else {
                x = 4;
...
//            std::cout << snake_ptr->body.positions[j][X_COORDINATE];
            if (snake_ptr->body.positions[j][X_COORDINATE] * tile_size_.x == tile_map_[i].getPosition().x &&
                ...
                int x;
                int y;
                int x_dif_next_current =
                        snake_ptr->body.positions[j + 1][X_COORDINATE] - snake_ptr->body.positions[j][X_COORDINATE];
                int x_dif_current_prev =
                        snake_ptr->body.positions[j][X_COORDINATE] - snake_ptr->body.positions[j - 1][X_COORDINATE];
                int y_dif_next_current =
                        snake_ptr->body.positions[j + 1][Y_COORDINATE] - snake_ptr->body.positions[j][Y_COORDINATE];
                int y_dif_current_prev =
                        snake_ptr->body.positions[j][Y_COORDINATE] - snake_ptr->body.positions[j - 1][Y_COORDINATE];
                if (x_dif_next_current == 0) {
                    if (y_dif_next_current == 1) {
                        if (x_dif_current_prev == 1) {
                            x = 2;
                            y = 0;
                        } else if (x_dif_current_prev == -1) {
                            x = 0;
                            y = 0;
                        } else if (x_dif_current_prev == 0) {
                            x = 2;
                            y = 1;
                        }
                    ...
                    if (y_dif_current_prev == 1) {
                        x = 0;
                        y = 1;
                    } else if (y_dif_current_prev == 0) {
                        x = 1;
                        y = 0;
                    } else if (y_dif_current_prev == -1) {
                        x = 0;
                        y = 0;
                    }
                } else if (x_dif_next_current == -1) {
        ...
        if (snake_ptr->body.positions[snake_ptr->body.length - 1][X_COORDINATE] * tile_size_.x ==
            tile_map_[i].getPosition().x && ///tail
            snake_ptr->body.positions[snake_ptr->body.length - 1][Y_COORDINATE] * tile_size_.y ==
            tile_map_[i].getPosition().y) {
            tile_map_[i].setRotation(0);

            int x;
            int y;
            int x_dif_next_current = snake_ptr->body.positions[snake_ptr->body.length - 2][X_COORDINATE] -
                                     snake_ptr->body.positions[snake_ptr->body.length - 1][X_COORDINATE];
            int y_dif_next_current = snake_ptr->body.positions[snake_ptr->body.length - 2][Y_COORDINATE] -
                                     snake_ptr->body.positions[snake_ptr->body.length - 1][Y_COORDINATE];
            if (x_dif_next_current == 0) {
                if (y_dif_next_current == 1) {
                    x = 4;
                    y = 3;
...
void Game::RenderLoop() {
    ...
    while (window_.isOpen()) {
        ...
        //////////////////////////
        if (snake2_ptr_ != nullptr || snakes_.size() != 0) {
            if (snakes_.size() != 0) {
                if (snake_die) {
                    if (snake2_die) {
                        if (snakes_.size() == 1)
                            GameOver("Crazy Snake");
                    }
                    else if (snakes_.empty()) GameOver("number-Snake");
                } else if (snake2_die) {
                    if (snake_die) {
                        if (snakes_.size() == 1)
                            GameOver("Crazy Snake");
                    }
                    else if (snakes_.empty()) GameOver("letter-Snake");
                }
...
////////////////////////////////////////////////
        ...
        if (!snake_die) {
            threads.push_back(std::thread([this]() { RenderSnake(snake_ptr_); }));
        }
        if (snake2_ptr_ != nullptr) {
            if (!snake2_die) {
                threads.push_back(std::thread([this]() { RenderSnake(snake2_ptr_); }));
            }
        }
        if (snakes_.size() != 0) {
            for (auto &snake: snakes_) {
                threads.push_back(std::thread([this, &snake]() { RenderSnake(&snake); }));
            }
        }
        for (auto &th: threads)
            th.join();
